...
function server(opts) {
  ...
  function checkPhp(bin) {
    return new Promise((resolve, reject) => {
      sh.silentExec(bin, ['-v'])
        .then(out => {
          const log = out.stdout.trim().toLowerCase();
          if (log.includes('macos will not include php')) {
            console.log('⚠️  Built-in PHP will be removed from future versions of macOS')
            return resolve()
          } else if (log.startsWith('php')) {
            return resolve()
          } else {
